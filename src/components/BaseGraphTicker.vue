<template>
  <section class="relative">
    <h3 class="text-lg leading-6 font-medium text-white my-8">
      {{ selectedTicker.ticker }} - USD
    </h3>
    <div
      class="flex items-end border-white border-b border-l h-64"
      ref="graph"
    >
      <div
        ...
        :key="`graph-${i}-${bar}`"
        :style="{ height: `${bar}%` }"
        class="graph-element bg-lime-500 border w-10"
      ></div>
    </div>
    <button
      type="button"
...
export default {
  ...
  computed: {
    normalizedGraph() {
      ...
      const minValue = Math.min(...this.updateMaxGraphElements);

      if (maxValue === minValue) {
        return this.updateMaxGraphElements.map(() => 50);
      }

      return this.updateMaxGraphElements.map((price) => {
        return 5 + ((price - minValue) * 95) / (maxValue - minValue);
      });
    },
    updateMaxGraphElements() {
      if (this.graph.length > this.maxGraphElements) {
        return this.graph.slice(`-${this.maxGraphElements}`);
      }
      return this.graph;
    },
  },
